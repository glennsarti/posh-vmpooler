function New-VMPoolerToken {
  ...
  Process {
    try {
      $result = Invoke-VMPoolerAPI -Method 'POST' -route 'token'
      
      if ($result.token -ne '') { Get-VMPoolerTokenDetail -Token $result.token}
    }
    catch {
      Throw "Unable to get a new token $_"
    }
  }
